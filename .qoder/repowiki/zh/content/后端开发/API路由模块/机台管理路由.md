# 机台管理路由

<cite>
**本文档引用的文件**
- [machines.py](file://backend/routers/machines.py)
- [plc_client.py](file://backend/plc_client.py)
- [models.py](file://backend/models.py)
- [schemas.py](file://backend/schemas.py)
- [main.py](file://backend/main.py)
- [config.py](file://backend/config.py)
- [database.py](file://backend/database.py)
- [dependencies.py](file://backend/dependencies.py)
- [logger.py](file://backend/utils/logger.py)
- [database_schema.sql](file://backend/database_schema.sql)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

机台管理路由是挤出机工艺参数管理系统的核心模块，负责机台设备的全生命周期管理。该系统实现了完整的CRUD操作，包括机台的添加、编辑、删除和查询功能。同时，系统集成了PLC连接管理机制，支持实时数据读取和写入操作，为工业自动化提供了完整的解决方案。

系统采用FastAPI框架构建，使用SQLAlchemy进行数据库操作，通过Snap7库与西门子PLC设备通信。整个架构遵循RESTful API设计原则，提供了安全的认证授权机制和详细的日志记录功能。

**更新** 新增了单个PLC参数写入功能，支持Int、Real、Bool类型的参数写入，包含类型转换和错误处理机制。该功能通过PLCWriteRequest模式实现，允许精确控制单个参数的写入操作。

## 项目结构

系统采用分层架构设计，主要包含以下核心目录和文件：

```mermaid
graph TB
subgraph "后端应用结构"
A[main.py] --> B[routers/]
A --> C[models.py]
A --> D[schemas.py]
A --> E[database.py]
A --> F[config.py]
B --> G[machines.py]
B --> H[parameters.py]
B --> I[products.py]
B --> J[templates.py]
B --> K[logs.py]
L[utils/] --> M[logger.py]
N[database_schema.sql] --> O[MySQL数据库]
P[config.py] --> Q[环境配置]
end
subgraph "前端应用"
R[frontend/]
S[index.html]
T[vue.global.js]
end
A --> R
```

**图表来源**
- [main.py:1-77](file://backend/main.py#L1-L77)
- [machines.py:1-275](file://backend/routers/machines.py#L1-L275)

**章节来源**
- [main.py:1-77](file://backend/main.py#L1-L77)
- [database_schema.sql:1-162](file://backend/database_schema.sql#L1-L162)

## 核心组件

### 机台路由模块

机台路由模块位于`backend/routers/machines.py`，提供了完整的机台管理API接口。该模块包含以下核心功能：

- **CRUD操作**：机台的创建、读取、更新、删除
- **状态监控**：实时检查PLC连接状态
- **参数读取**：从PLC设备批量读取工艺参数
- **参数写入**：向PLC设备批量写入控制参数
- **单个参数写入**：支持Int、Real、Bool类型的单个参数写入
- **模板绑定**：将工艺模板与机台进行关联

### PLC客户端类

PLC客户端类位于`backend/plc_client.py`，封装了与西门子PLC设备的所有通信逻辑：

- **连接管理**：建立、维护和断开PLC连接
- **数据读取**：支持多种数据类型的参数读取
- **数据写入**：支持多种数据类型的参数写入
- **地址解析**：解析和验证PLC地址格式
- **错误处理**：完善的异常处理和错误恢复机制

### 数据模型定义

系统使用SQLAlchemy ORM定义了完整的数据模型，包括：

- **机台模型**：存储机台基本信息和连接配置
- **模板模型**：定义可复用的工艺参数模板
- **参数模型**：管理模板参数和工艺记录参数
- **日志模型**：记录所有操作的详细信息

**章节来源**
- [machines.py:12-275](file://backend/routers/machines.py#L12-L275)
- [plc_client.py:6-188](file://backend/plc_client.py#L6-L188)
- [models.py:19-142](file://backend/models.py#L19-L142)

## 架构概览

系统采用分层架构设计，各层职责明确，耦合度低：

```mermaid
graph TB
subgraph "表现层"
A[前端Web界面]
B[移动端应用]
end
subgraph "API网关层"
C[FastAPI应用]
D[路由处理器]
E[中间件]
end
subgraph "业务逻辑层"
F[机台管理服务]
G[参数管理服务]
H[模板管理服务]
I[日志管理服务]
end
subgraph "数据访问层"
J[SQLAlchemy ORM]
K[数据库连接池]
end
subgraph "设备通信层"
L[PLC客户端]
M[Snap7库]
N[网络通信]
end
subgraph "数据存储层"
O[MySQL数据库]
P[日志文件系统]
end
A --> C
B --> C
C --> D
D --> F
D --> G
D --> H
D --> I
F --> J
G --> J
H --> J
I --> J
J --> K
K --> O
F --> L
L --> M
M --> N
N --> P
```

**图表来源**
- [main.py:12-31](file://backend/main.py#L12-L31)
- [machines.py:1-10](file://backend/routers/machines.py#L1-L10)

## 详细组件分析

### 机台CRUD操作实现

#### 机台创建功能

机台创建功能实现了完整的数据验证和持久化逻辑：

```mermaid
sequenceDiagram
participant Client as 客户端
participant Router as 机台路由
participant DB as 数据库
participant Logger as 日志服务
Client->>Router : POST /api/machines
Router->>Router : 验证JWT令牌
Router->>DB : 检查机台编号唯一性
DB-->>Router : 返回检查结果
alt 机台编号已存在
Router-->>Client : 400 错误响应
else 唯一性验证通过
Router->>DB : 创建机台记录
DB-->>Router : 返回新机台对象
Router->>Logger : 记录创建日志
Logger-->>Router : 日志记录完成
Router-->>Client : 201 成功响应
end
```

**图表来源**
- [machines.py:32-47](file://backend/routers/machines.py#L32-L47)
- [dependencies.py:38-50](file://backend/dependencies.py#L38-L50)

#### 机台查询功能

机台查询功能支持分页查询和模板信息关联：

```mermaid
flowchart TD
A[接收查询请求] --> B[验证JWT令牌]
B --> C[统计总记录数]
C --> D[计算偏移量]
D --> E[执行分页查询]
E --> F[设置默认状态]
F --> G{是否有关联模板?}
G --> |是| H[添加模板ID和名称]
G --> |否| I[设置模板ID和名称为空]
H --> J[返回结果]
I --> J
J --> K[返回JSON响应]
```

**图表来源**
- [machines.py:14-30](file://backend/routers/machines.py#L14-L30)

#### 机台更新功能

机台更新功能实现了选择性字段更新：

```mermaid
flowchart TD
A[接收更新请求] --> B[验证JWT令牌]
B --> C[查询目标机台]
C --> D{机台是否存在?}
D --> |否| E[返回404错误]
D --> |是| F[遍历更新字段]
F --> G[设置属性值]
G --> H[提交事务]
H --> I[刷新机台对象]
I --> J[记录更新日志]
J --> K[返回更新后的机台]
```

**图表来源**
- [machines.py:49-65](file://backend/routers/machines.py#L49-L65)

#### 机台删除功能

机台删除功能包含了级联删除和日志记录：

```mermaid
flowchart TD
A[接收删除请求] --> B[验证JWT令牌]
B --> C[查询目标机台]
C --> D{机台是否存在?}
D --> |否| E[返回404错误]
D --> |是| F[记录删除前名称]
F --> G[执行删除操作]
G --> H[提交事务]
H --> I[记录删除日志]
I --> J[返回成功消息]
```

**图表来源**
- [machines.py:67-81](file://backend/routers/machines.py#L67-L81)

### PLC连接管理机制

#### 连接建立流程

PLC连接管理实现了自动化的连接建立和维护：

```mermaid
sequenceDiagram
participant Router as 路由处理器
participant PLC as PLC客户端
participant S7 as Snap7库
participant PLCDevice as PLC设备
Router->>PLC : 创建PLC实例
PLC->>PLC : 初始化连接参数
Router->>PLC : 调用connect()
PLC->>S7 : 创建Client实例
S7->>PLCDevice : 发起TCP连接
PLCDevice-->>S7 : 连接确认
S7-->>PLC : 连接状态
PLC-->>Router : 返回连接结果
Router->>PLC : 执行业务操作
Router->>PLC : 调用disconnect()
PLC->>S7 : 断开连接
```

**图表来源**
- [plc_client.py:24-31](file://backend/plc_client.py#L24-L31)

#### 心跳检测机制

系统实现了基于连接状态的心跳检测：

```mermaid
flowchart TD
A[启动状态检查] --> B[查询所有机台]
B --> C[遍历每个机台]
C --> D{机台是否有IP地址?}
D --> |否| E[设置为离线]
D --> |是| F[创建PLC客户端]
F --> G[尝试连接]
G --> H{连接成功?}
H --> |是| I[设置为在线]
H --> |否| J[设置为离线]
I --> K[记录连接状态]
J --> K
K --> L[释放PLC连接]
L --> M[继续下一个机台]
M --> N{还有机台吗?}
N --> |是| C
N --> |否| O[返回状态列表]
```

**图表来源**
- [machines.py:83-105](file://backend/routers/machines.py#L83-L105)

#### 断线重连处理

PLC客户端实现了智能的断线检测和重连机制：

```mermaid
flowchart TD
A[检测连接状态] --> B{槽位是否变化?}
B --> |是| C[更新槽位]
B --> |否| D{连接是否有效?}
C --> E[断开旧连接]
E --> F[重新建立连接]
F --> G[返回连接状态]
D --> |否| G
D --> |是| H[断开并重新连接]
H --> G
```

**图表来源**
- [plc_client.py:42-49](file://backend/plc_client.py#L42-L49)

### 参数读取和写入实现

#### 参数读取流程

参数读取功能支持批量操作和模板驱动：

```mermaid
sequenceDiagram
participant Client as 客户端
participant Router as 机台路由
participant DB as 数据库
participant PLC as PLC客户端
participant S7 as Snap7库
Client->>Router : GET /api/machines/{id}/read
Router->>Router : 验证JWT令牌
Router->>DB : 查询机台信息
DB-->>Router : 返回机台对象
Router->>DB : 查询关联模板
DB-->>Router : 返回模板参数
Router->>PLC : 创建PLC客户端
Router->>PLC : 连接PLC设备
PLC->>S7 : 建立连接
S7-->>PLC : 连接成功
loop 遍历模板参数
Router->>PLC : 读取单个参数
PLC->>S7 : 发送读取请求
S7-->>PLC : 返回参数值
PLC-->>Router : 返回参数值
Router->>Router : 组装响应数据
end
Router->>PLC : 断开连接
Router-->>Client : 返回读取结果
```

**图表来源**
- [machines.py:107-183](file://backend/routers/machines.py#L107-L183)

#### 单个参数写入流程

**更新** 新增的单个PLC参数写入功能支持Int、Real、Bool类型的参数写入：

```mermaid
sequenceDiagram
participant Client as 客户端
participant Router as 机台路由
participant DB as 数据库
participant PLC as PLC客户端
participant S7 as Snap7库
Client->>Router : POST /api/machines/{id}/write
Router->>Router : 验证JWT令牌
Router->>DB : 查询机台信息
DB-->>Router : 返回机台对象
Router->>DB : 查询模板参数槽位映射
DB-->>Router : 返回槽位映射
Router->>PLC : 创建PLC客户端
Router->>PLC : 连接PLC设备
PLC->>S7 : 建立连接
S7-->>PLC : 连接成功
loop 遍历请求参数
Router->>Router : 获取槽位信息
Router->>Router : 类型转换 (Int/Real/Bool)
Router->>PLC : 写入单个参数
PLC->>S7 : 发送写入请求
S7-->>PLC : 写入确认
PLC-->>Router : 返回写入结果
Router->>Router : 记录写入状态
end
Router->>PLC : 断开连接
Router-->>Client : 返回写入结果
```

**图表来源**
- [machines.py:185-237](file://backend/routers/machines.py#L185-L237)

#### 批量参数写入流程

批量参数写入功能支持多个参数的批量写入操作：

```mermaid
sequenceDiagram
participant Client as 客户端
participant Router as 机台路由
participant DB as 数据库
participant PLC as PLC客户端
participant S7 as Snap7库
Client->>Router : POST /api/machines/{id}/write
Router->>Router : 验证JWT令牌
Router->>DB : 查询机台信息
DB-->>Router : 返回机台对象
Router->>DB : 查询模板参数
DB-->>Router : 返回槽位映射
Router->>PLC : 创建PLC客户端
Router->>PLC : 连接PLC设备
PLC->>S7 : 建立连接
S7-->>PLC : 连接成功
loop 遍历请求参数
Router->>Router : 获取槽位信息
Router->>Router : 类型转换 (Int/Real/Bool)
Router->>PLC : 写入单个参数
PLC->>S7 : 发送写入请求
S7-->>PLC : 写入确认
PLC-->>Router : 返回写入结果
Router->>Router : 记录写入状态
end
Router->>PLC : 断开连接
Router-->>Client : 返回写入结果
```

**图表来源**
- [machines.py:185-237](file://backend/routers/machines.py#L185-L237)

#### 数据类型转换机制

PLC参数读写支持多种数据类型，包括新增的单个参数写入功能：

| 数据类型 | 地址前缀 | 字节大小 | 用途 | 类型转换 |
|---------|---------|---------|------|----------|
| Int | VW, VC | 2字节 | 整数值 | int(float(value)) |
| Real | VD | 4字节 | 浮点数值 | float(value) |
| Bool | VB, V.x, C, T | 1字节/位 | 布尔值 | bool(value) 或字符串转布尔 |
| Bit | V.x | 1位 | 位操作 | 支持位级别写入 |
| Counter | C | 2字节 | 计数器 | 支持计数器写入 |
| Timer | T | 2字节 | 定时器 | 支持定时器写入 |

**更新** 新增的单个参数写入功能包含以下类型转换逻辑：
- **Int类型**：使用`int(float(value))`确保数值转换的稳定性
- **Real类型**：使用`float(value)`进行浮点数转换
- **Bool类型**：支持布尔值直接转换，或字符串形式如"true"、"1"、"yes"、"on"的转换

**章节来源**
- [plc_client.py:51-180](file://backend/plc_client.py#L51-L180)
- [plc_client.py:115-180](file://backend/plc_client.py#L115-L180)
- [schemas.py:200-207](file://backend/schemas.py#L200-L207)

### 机台状态监控和实时数据获取

#### 实时状态监控

系统提供了两种状态监控方式：

1. **批量状态检查**：一次性检查所有机台的连接状态
2. **单机台状态查询**：针对特定机台的状态检查

#### 实时数据获取

实时数据获取功能支持：

- **批量参数读取**：一次请求获取多个参数值
- **模板驱动读取**：根据机台绑定的模板自动获取参数
- **单个参数写入**：支持Int、Real、Bool类型的单个参数写入
- **错误处理**：完善的异常捕获和错误报告机制

**章节来源**
- [machines.py:83-105](file://backend/routers/machines.py#L83-L105)
- [machines.py:107-183](file://backend/routers/machines.py#L107-L183)
- [machines.py:185-237](file://backend/routers/machines.py#L185-L237)

### 机台配置管理和设备发现

#### 设备发现机制

系统通过以下方式实现设备发现：

1. **IP地址配置**：在机台配置中指定PLC IP地址
2. **连接状态检测**：通过连接尝试判断设备可用性
3. **参数模板绑定**：通过模板定义设备参数集合

#### 连接测试功能

连接测试功能提供了完整的连接验证：

```mermaid
flowchart TD
A[开始连接测试] --> B[创建PLC客户端]
B --> C[设置连接参数]
C --> D[尝试建立连接]
D --> E{连接成功?}
E --> |是| F[返回连接成功]
E --> |否| G[返回连接失败]
F --> H[关闭连接]
G --> H
H --> I[返回测试结果]
```

**图表来源**
- [machines.py:92-101](file://backend/routers/machines.py#L92-L101)

**章节来源**
- [machines.py:239-275](file://backend/routers/machines.py#L239-L275)

## 依赖关系分析

系统采用了清晰的依赖注入和模块化设计：

```mermaid
graph TB
subgraph "外部依赖"
A[FastAPI]
B[SQLAlchemy]
C[Snap7]
D[PyMySQL]
E[JWT]
end
subgraph "内部模块"
F[main.py]
G[routers/]
H[models.py]
I[schemas.py]
J[database.py]
K[config.py]
L[utils/]
end
subgraph "路由模块"
M[machines.py]
N[parameters.py]
O[products.py]
P[templates.py]
Q[logs.py]
end
A --> F
B --> H
C --> M
D --> J
E --> K
F --> G
G --> M
G --> N
G --> O
G --> P
G --> Q
M --> H
M --> I
M --> J
M --> K
M --> L
N --> H
O --> H
P --> H
Q --> H
```

**图表来源**
- [main.py:10-31](file://backend/main.py#L10-L31)
- [machines.py:1-10](file://backend/routers/machines.py#L1-L10)

**章节来源**
- [main.py:1-77](file://backend/main.py#L1-L77)
- [dependencies.py:1-50](file://backend/dependencies.py#L1-L50)

## 性能考虑

### 数据库性能优化

系统采用了多项数据库性能优化策略：

- **连接池管理**：使用`pool_pre_ping=True`确保连接有效性
- **索引优化**：为常用查询字段建立索引
- **分页查询**：支持大数据量的分页浏览
- **延迟加载**：使用`joinedload`优化关联查询

### PLC通信优化

PLC通信层面的优化包括：

- **连接复用**：在同一请求中复用PLC连接
- **批量操作**：支持批量参数读写减少通信次数
- **错误缓存**：避免重复的连接尝试
- **超时控制**：合理的超时设置防止阻塞

### 缓存策略

系统实现了多层缓存策略：

- **内存缓存**：缓存常用的配置信息
- **数据库缓存**：使用连接池减少数据库连接开销
- **会话缓存**：缓存用户认证信息

## 故障排除指南

### 常见问题诊断

#### 机台连接失败

**症状**：PLC连接状态显示为"离线"

**可能原因**：
1. IP地址配置错误
2. 网络连接问题
3. PLC设备故障
4. 认证信息错误

**解决步骤**：
1. 验证机台IP地址配置
2. 检查网络连通性
3. 确认PLC设备电源状态
4. 验证Rack和Slot配置

#### 参数读取失败

**症状**：读取参数时返回空值或错误

**可能原因**：
1. 参数地址格式错误
2. 数据类型不匹配
3. PLC设备未就绪
4. 权限不足

**解决步骤**：
1. 验证参数地址格式
2. 检查数据类型设置
3. 确认PLC设备状态
4. 验证模板绑定情况

#### 单个参数写入失败

**更新** 新增的单个PLC参数写入功能的故障排除：

**症状**：单个参数写入返回False或抛出异常

**可能原因**：
1. 参数地址格式错误
2. 数据类型转换失败
3. 槽位配置不正确
4. PLC设备未就绪
5. 权限不足

**解决步骤**：
1. 验证参数地址格式（支持VW、VD、VB、VC、V.x、C、T等）
2. 检查数据类型转换（Int: int(float(value)), Real: float(value), Bool: bool(value) 或字符串转布尔）
3. 确认槽位配置（模板槽位优先于机台槽位）
4. 验证PLC设备状态
5. 检查模板绑定情况

#### 模板绑定问题

**症状**：模板绑定后无法读取参数

**可能原因**：
1. 模板参数配置错误
2. 机台槽位设置不匹配
3. 参数地址冲突

**解决步骤**：
1. 检查模板参数配置
2. 验证机台槽位设置
3. 确认参数地址唯一性

### 日志分析

系统提供了详细的日志记录功能，便于问题诊断：

- **操作日志**：记录所有用户操作
- **PLC通信日志**：记录PLC读写操作
- **系统错误日志**：记录系统异常信息

**章节来源**
- [logger.py:89-120](file://backend/utils/logger.py#L89-L120)
- [logger.py:195-199](file://backend/utils/logger.py#L195-L199)

## 结论

机台管理路由模块为挤出机工艺参数管理系统提供了完整的核心功能。系统通过清晰的架构设计、完善的错误处理机制和丰富的监控功能，为企业级工业自动化应用提供了可靠的解决方案。

主要特点包括：

- **完整的CRUD操作**：支持机台的全生命周期管理
- **智能PLC集成**：提供稳定的设备通信能力
- **灵活的参数管理**：支持模板驱动的参数配置
- **完善的日志系统**：提供详细的操作审计功能
- **安全的认证机制**：确保系统的安全性
- **新增单个参数写入功能**：支持Int、Real、Bool类型的精确参数控制

**更新** 新增的单个PLC参数写入功能为系统增加了更精细的控制能力，支持不同数据类型的精确写入和类型转换，为工业自动化提供了更强大的工具。该功能通过PLCWriteRequest模式实现，允许用户精确控制单个参数的写入操作，并包含完整的异常处理机制，确保系统的稳定性和可靠性。

该系统为后续的功能扩展和性能优化奠定了良好的基础，能够满足现代工业自动化系统的需求。